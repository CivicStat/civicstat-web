...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    const m = await getScopedMotion(city, id);
...
export default async function GemeenteMotieDetailPage({ params }: Props) {
  const { city, id } = await params;
  ...
  let motion;
  try {
    motion = await getScopedMotion(city, id);
  } catch {
    return (
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-7 pb-24">
      ...
      <nav className="text-[11px] text-text-tertiary mb-4 flex items-center gap-1.5">
        ...
        <span>/</span>
        <Link href={r.moties} className="hover:text-moss transition-colors">Moties</Link>
        <span>/</span>
                ...
                <div className="mt-4 pt-4 border-t border-border-subtle">
                  ...
                  <div className="space-y-1.5">
                    {(vote.rawData.Stemming as RawStemming[])
                      .sort((a, b) => b.FractieGrootte - a.FractieGrootte)
                      .map((s) => (
                        <div
                          key={s.Id}
                  ...
                  <div className="max-h-[300px] overflow-y-auto space-y-1">
                    {(vote.records as VoteRecord[])
                      .sort((a, b) => {
                        ...
                        return order[a.voteValue] - order[b.voteValue];
                      })
                      .map((rec) => (
                        <div
                          key={rec.id}
