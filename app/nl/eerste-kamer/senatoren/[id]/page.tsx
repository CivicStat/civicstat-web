...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    const member = await getScopedMember(SLUG, id);
    return {
      title: `${member.surname} — Eerste Kamer — CivicStat`,
      description: `Stemgedrag en moties van senator ${member.name} in de Eerste Kamer.`,
    };
  } catch {
    return { title: "Senator — CivicStat" };
  }
}

export default async function SenatorDetailPage({ params }: Props) {
  const { id } = await params;
  ...
  let member;
  try {
    member = await getScopedMember(SLUG, id);
  } catch {
    return (
      <div className="mx-auto max-w-[1200px] px-5 py-7 pb-24">
        <Link
          href={routes.ek.senatoren}
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-7 pb-24">
      ...
      <nav className="text-[11px] text-text-tertiary mb-4 flex items-center gap-1.5">
        ...
        <span>/</span>
        <Link href={routes.ek.senatoren} className="hover:text-moss transition-colors">Senatoren</Link>
        <span>/</span>
      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div>
          ...
          <div className="mt-2">
            <Link href={routes.ek.partij(member.party.id)}>
      ...
      {motions.length > 0 && (
        <section>
          ...
          <p className="text-[12px] text-text-tertiary mt-3">
            Moties waarbij deze senator als indiener of mede-indiener betrokken is.
          </p>
        </section>
      )}
    </div>
